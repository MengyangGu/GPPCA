...
R0_list=list(1:p)
for(i_p in 1:p){
  R0_list[[i_p]]=as.matrix(abs(outer(X[,i_p],X[,i_p],'-')))
}

R0_star_list=list(1:p)
for(i_p in 1:p){
  R0_star_list[[i_p]]=as.matrix(abs(outer(Xt[,i_p],Xt[,i_p],'-')))
}

r0_list=list(1:p)
for(i_p in 1:p){
  r0_list[[i_p]]=as.matrix(abs(outer(X[,i_p],Xt[,i_p],'-')))
}

# Xt.array<-array(data=rep(0,n*dim(Xt)[2]*n),dim=c(n,dim(Xt)[2],n))
# for(i in 1:n){
#   for(j in 1:dim(Xt)[2]){
#     Xt.array[,j,i]<-abs(Xt[,j]-Xt[i,j])
#     }
#   }
...
logpi<-function(z){
  r<-exp(z)
  A=separable_kernel(R0_list,r[1:p],kernel_type="matern_5_2",alpha=rep(2,p))
  A=A+r[14]*diag(n)
  # A<-0*diag(n)
  # for(j in 1:dim(X)[2]){
  #   A<-A-r[j]*X.array[,j,]}
...
At=separable_kernel(R0_star_list,r[1:p],kernel_type="matern_5_2",alpha=rep(2,p))
At<-At+r[14]*diag(n)
...
#   T<-T-r[j]*T.array[,j,]}
# T<-exp(T)
T=separable_kernel(r0_list,r[1:p],kernel_type="matern_5_2",alpha=rep(2,p))
